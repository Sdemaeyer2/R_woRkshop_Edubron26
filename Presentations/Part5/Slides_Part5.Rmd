...
---
<div class="my-logo-right"> <img src="broom_logo.png" width="80%"/> </div>

## The .UA-red[`broom`]  package (part of the tidyverse)

Creates .SW-greenD[**tibbles**] based on results from a statistical analysis!

Function .UA-red[**`tidy()`**] represents output of an analysis as a tibble

```{r}
library(broom)

C <- cor.test(penguins$flipper_length_mm, penguins$body_mass_g)
tidy(C)
...
.pull-left[
Compare the `body_mass_g` of the "Gentoo" and "Adelie" penguins 
...
Result: **tidy data set** with information on **parameter estimates**
...
Result: tidy data set with information on **model fit**
...
Result: add information to the data set based on the model like **fitted values**, **residuals**, ...
...
Create an **html-table** that presents the estimates of two models: Model1 & Model2
