...
table(
  transition_data_cleaned$Educ_level, 
  transition_data_cleaned$Educ_level_cat
  )
table(
  transition_data_cleaned$Gender, 
  transition_data_cleaned$Gender_cat
  )
table(
  transition_data_cleaned$SES_language, 
  transition_data_cleaned$SES_language_cat
  )
table(
  transition_data_cleaned$SES_neighbourhood,
  transition_data_cleaned$SES_neighbourhood_cat
  )
table(
  transition_data_cleaned$SES_educ_level,
...
descriptives %>%
  mutate_all(round, 2) %>%
  mutate(
    Name = c("IQ score", "Reading score", "Mathematics score")
    ) %>%
  relocate(Name, .before = "mean") %>%
  kable(
...
#| echo: true
Model_lm1 <- lm(
  Math_Z ~ Auto_mot_Z + Contr_mot_Z + Amot_Z,
...
Model_lm2 <- lm(
  Math_Z ~ IQ_Z + Reading_Z + Gender + Educ_level_cat + 
...
Model_lm2_B <- lm(
  Math_Z ~ IQ_score + Reading_score + Gender + Educ_level_cat +
    Auto_mot_Z + Contr_mot_Z + Amot_Z,
  data = transition_data_omitted
  )
```
