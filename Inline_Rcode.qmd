...
The penguins live on three islands: Biscoe (*n* = `r islands_list$Biscoe$n`), Dream (*n* = `r islands_list$Dream$n`), and Torgersen (*n* = `r islands_list$Torgersen$n`).
...
list_lm <- tidy(Model_lm, conf.int = TRUE) %>%
  ...
  mutate(
    term = ifelse(term == "(Intercept)", yes = "Intercept", no = term)
    ) %>%
